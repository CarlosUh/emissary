...
func (d DataMap) Get(key string, defaultValue interface{}) string {
...
func (d DataMap) parseCondition(condition string, key string) (string, error) {
...
func (d DataMap) evaluateBlock(block string) bool {
	if block == "[else]" {
		return true
	}
...
func (d DataMap) getParsedVal(key string) string {
	var err error
	...
	for _, m := range matches {
		...
		v := getNestedProperty(map[string]interface{}(d), k)
